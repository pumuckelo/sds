...
export const Ref = Schema.String.check(Schema.isPattern(/^[A-Za-z0-9_-]{4,21}$/))
export const Title = Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(240), Schema.isPattern(/\S/))
...
export const Status = Schema.Literals(['queued', 'active', 'blocked', 'done', 'cancelled'])
...
export const Todo = Schema.Struct({ id: Id, title: Title, status: TodoStatus, stage: Schema.optional(Stage) })
export const Finding = Schema.Struct({
  id: Id, description: Text, severity: Severity,
  status: Schema.Literals(['open', 'resolved', 'dismissed']), file: Schema.optional(Title),
  line: Schema.optional(Revision), resolution: Schema.optional(Text),
})
...
export const Project = Schema.Struct({ schemaVersion: Schema.Literal(1), id: Id, name: Title })
export const Checkout = Schema.Struct({ id: Id, projectId: Id, name: Title, path: Schema.NonEmptyString })
export type Checkout = typeof Checkout.Type
export const Registry = Schema.Struct({ schemaVersion: Schema.Literal(1), checkouts: Schema.Array(Checkout) })
...
export const Operation = Schema.Union([
  ...
  Schema.Struct({ type: Schema.Literal('status.set'), status: Status, reason: Schema.optional(Text) }),
  Schema.Struct({ type: Schema.Literal('todo.add'), title: Title, stage: Schema.optional(Stage) }),
  Schema.Struct({ type: Schema.Literal('todo.update'), todoId: Ref, title: Schema.optional(Title), status: Schema.optional(TodoStatus), stage: Schema.optional(Stage) }),
  Schema.Struct({ type: Schema.Literal('todo.complete'), todoIds: Schema.Array(Ref).check(Schema.isMinLength(1), Schema.isMaxLength(200)) }),
  Schema.Struct({ type: Schema.Literal('finding.add'), description: Schema.NonEmptyString.check(Schema.isMaxLength(100_000)), severity: Severity, file: Schema.optional(Title), line: Schema.optional(Revision) }),
  Schema.Struct({ type: Schema.Literal('finding.resolve'), findingId: Ref, status: Schema.Literals(['resolved', 'dismissed']), resolution: Schema.NonEmptyString.check(Schema.isMaxLength(100_000)) }),
  Schema.Struct({ type: Schema.Literal('note.add'), text: Schema.NonEmptyString.check(Schema.isMaxLength(100_000)) }),
])
...
export const GetManyInput = Schema.Struct({ checkoutId: Ref, taskIds: Schema.Array(Ref).check(Schema.isMinLength(1), Schema.isMaxLength(100)), view: GetInput.fields.view, offset: GetInput.fields.offset, limit: GetInput.fields.limit })
export const UpdateItem = Schema.Struct({ taskId: Ref, expectedRevision: Revision, operations: UpdateInput.fields.operations })
export const UpdateManyBody = Schema.Struct({ updates: Schema.Array(UpdateItem).check(Schema.isMinLength(1), Schema.isMaxLength(100)), verbose: Schema.optional(Schema.Boolean) })
export const UpdateManyInput = Schema.Struct({ checkoutId: Ref, ...UpdateManyBody.fields })
