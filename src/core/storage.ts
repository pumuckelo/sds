...
export const failure = (code: ErrorCode, message: string) =>
  new SdsError({ code, message });
export const io = <A>(label: string, action: () => Promise<A>) =>
  Effect.tryPromise({
    try: () => action(),
    catch: (error) =>
      error instanceof SdsError
        ? error
        : failure(
            "STORAGE_ERROR",
...
export const decode = <
  S extends Schema.Constraint & { DecodingServices: never },
>(
  schema: S,
  input: unknown,
) =>
  Schema.decodeUnknownEffect(schema)(input, { onExcessProperty: "error" }).pipe(
    Effect.mapError((error) => failure("INVALID_INPUT", error.message)),
  );

export const resolveRef = Effect.fn("resolveRef")(function* <
  T extends { readonly id: string },
>(items: readonly T[], ref: string) {
  const exact = items.find((item) => item.id === ref);
  if (exact) return exact;
  const matches = items.filter((item) => item.id.startsWith(ref));
  if (matches.length === 0)
    return yield* failure("NOT_FOUND", `No match for ${ref}`);
  if (matches.length !== 1)
    return yield* failure(
      "AMBIGUOUS_REF",
      `Ambiguous reference ${ref}; use one of: ${matches.map((item) => item.id).join(", ")}`,
    );
  return matches[0]!;
});
export function shortRef(
  ...
  while (
    size < id.length &&
    siblings.some(
      (other) => other.id !== id && other.id.startsWith(id.slice(0, size)),
    )
  )
    size++;
  ...
  static live(stateDir: string) {
    const storageIo = <A>(
      ...
      io(label, async () => {
        try {
          ...
          const errno = Schema.decodeUnknownOption(
            Schema.Struct({
              code: Schema.Literals(["EACCES", "EPERM", "EROFS", "ENOTDIR"]),
            }),
          )(error);
          if (errno._tag === "Some") {
            ...
            const isState =
              resolve(path) === state ||
              resolve(path).startsWith(`${state}${sep}`);
            ...
            throw failure(
              "STORAGE_ERROR",
              `${label}: ${errno.value.code} at ${path}. ${remedy}`,
            );
          }
          throw error;
        }
    ...
    return Layer.succeed(
      ...
      Storage.of({
        stateDir,
        canonical: (path) =>
          storageIo("Resolve checkout", path, async () => {
            const result = await realpath(path);
            if (!(await lstat(result)).isDirectory())
              throw failure(
                "INVALID_INPUT",
                "Checkout path must be a directory",
              );
            return result;
          }),
        ensureRepo: (path) =>
          storageIo("Prepare task directory", path, async () => {
            ...
            for (const dir of [root, join(root, "tasks")]) {
              ...
              if (stat.isSymbolicLink() || !stat.isDirectory())
                throw failure(
                  "STORAGE_ERROR",
                  `${dir} must be a real directory`,
                );
            }
            return root;
          }),
        read: (path) =>
          storageIo("Read JSON", path, async () => {
            try {
              if ((await lstat(path)).isSymbolicLink())
                throw failure("STORAGE_ERROR", `Refusing symlink: ${path}`);
        ...
        write: (path, data) =>
          storageIo("Write JSON", path, async () => {
            await mkdir(dirname(path), { recursive: true });
            const temp = join(
              dirname(path),
              `.${basename(path)}.${nanoid()}.tmp`,
            );
            try {
        ...
        taskFiles: (path) =>
          storageIo("List tasks", path, async () => {
            const files = await readdir(join(path, ".agent-work", "tasks"));
            return files
              .filter((file) => /^[A-Za-z0-9_-]{21}\.json$/.test(file))
              .sort();
          }),
        lock: (key, effect) =>
          Effect.scoped(
            Effect.gen(function* () {
              ...
              const release = yield* Effect.acquireRelease(
                storageIo("Acquire lock", lockPath, async () => {
                  await mkdir(dirname(lockPath), { recursive: true });
                  try {
                    return await lockfile.lock(lockPath, {
                      realpath: false,
                      stale: 30_000,
                      retries: { retries: 30, minTimeout: 25, maxTimeout: 100 },
                    });
                  } catch (error) {
                    if ((error as NodeJS.ErrnoException).code === "ELOCKED")
                      throw failure(
                        "BUSY",
                        "Another writer holds this task. Retry shortly.",
                      );
                    throw error;
                  }
                }),
                (release) =>
                  storageIo("Release lock", lockPath, release).pipe(
                    Effect.orDie,
                  ),
              );
              void release;
