...
const definitions = [
  ...
  { name: 'task_update', description: 'Apply semantic operations atomically to one task. parent.set moves a subtree; parentId:null detaches to top level. dependencies.set replaces prerequisite task refs (empty array clears). Self-dependencies, dependency cycles and cross-checkout links are rejected. Dependencies do not cascade status. expectedRevision is required: read again after CONFLICT. Complete many todos with one todo.complete operation. Returns counts and newly added IDs; verbose includes change descriptions. IDs accept unambiguous prefixes of at least four characters.', schema: S.UpdateInput, operation: 'update' },
