...
const definitions = [
  ...
  { name: 'task_create', description: 'Create a task with optional initial todos in one call. Returns IDs, short refs and revision.', schema: S.CreateInput, operation: 'create' },
  { name: 'task_list', description: 'List compact task summaries in one checkout, with filters and offset pagination.', schema: S.ListInput, operation: 'list' },
  { name: 'task_get', description: 'Read a task using a full ID or unambiguous prefix. Default working view includes pending todos, open findings and three latest notes. full includes all details; history is paginated, newest first.', schema: S.GetInput, operation: 'get' },
  { name: 'task_update', description: 'Apply semantic operations atomically to one task. expectedRevision is required: read again after CONFLICT. Complete many todos with one todo.complete operation. Returns counts and newly added IDs; verbose includes change descriptions. IDs accept unambiguous prefixes of at least four characters.', schema: S.UpdateInput, operation: 'update' },
