...
import { humanize, type TaskList } from '@/lib/api'

type Item = TaskList['items'][number]
export function TaskTree({ tasks, matches, onOpen }: { tasks: Item[]; matches: Item[]; onOpen: (id: string) => void }) {
  ...
  for (const task of matches) {
    ...
    while (parent && !seen.has(parent)) { seen.add(parent); shown.add(parent); parent = byId.get(parent)?.parentId ?? null }
  }
  ...
  for (const task of tasks) {
    ...
    const siblings = children.get(parent) ?? []; siblings.push(task); children.set(parent, siblings)
  }
  const filtering = matches.length !== tasks.length
  const rows: { task: Item; depth: number }[] = []
  const pending = (children.get(null) ?? []).map(task => ({ task, depth: 0 })).reverse()
  while (pending.length) {
    const row = pending.pop()!; rows.push(row)
    if (filtering || !collapsed.has(row.task.id)) for (const task of [...(children.get(row.task.id) ?? [])].reverse()) pending.push({ task, depth: row.depth + 1 })
  }
  if (!rows.length) return <Empty><EmptyHeader><EmptyTitle>No matching tasks</EmptyTitle><EmptyDescription>Create a task or adjust your filters.</EmptyDescription></EmptyHeader></Empty>
  return <div aria-label="Task hierarchy" className="flex flex-col gap-2">{rows.map(({ task, depth }) => <div key={task.id} className="flex items-center gap-2 rounded-lg border p-3" style={{ marginInlineStart: Math.min(depth, 12) * 20 }}>
